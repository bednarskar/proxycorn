...
public class CountryButton extends ToggleButton {
    ...
    final EventHandler<MouseEvent> countryFilterAddEventHandler = ME -> {
        ...
        if (obj.isPressed() && !obj.isSelected()) {
            ...
            obj.setStyle(ProjectConstants.PINK_SHADOW);
            obj.setSelected(false);
            LOGGER.debug(Filter.getInstance());
        } else {
            Filter.getInstance().removeCountry(obj.getId());
            obj.setStyle(ProjectConstants.NONE_EFFECT_STYLE);
            LOGGER.debug(Filter.getInstance());
        }
    };

    public CountryButton(String fileName) {
        String code = fileName.replace(PNG, EMPTY_STRING).replace(ProjectConstants.IMG_FLAGS_URL, EMPTY_STRING);
        this.setId(code);
        ...
        this.setStyle(ProjectConstants.BUTTON_COUNTRY_STYLE);
